...
> Plan de travail actualisé en cours de journée. Journal : [`2026-06-09.md`](./2026-06-09.md)
...
- Re-tests PartSmart post-fix Brian : `WFW5620` (7 modèles `whi`), `80040` (multi-catalogues), `whirlpool` → `[]` (normal)
- Réponse à Brian Fricke (LeadVenture) — excuses pour le délai, confirmation OK
...
- deep-link `?q=` sur `/recherche-par-modele` (front) — déjà implémenté
...
Fichier : [`2026-06-09.md`](./2026-06-09.md) — objectifs cochés en fin de journée.
...
| `midbec-go-api/internal/httpserver/handlers/pim.go` | Dans `GetPIMCategoryProducts` : lookup ERP par `sku` (helper `lookupERPBySKU`) |
...
- [x] Commits front UX (lundi) + journal journey
- [x] Vérifier / clôturer dette commits 5 juin
...
- [x] Smoke curl catégorie — 502 UnoPIM env ; `go build` OK
- [x] Re-tests PartSmart + réponse Brian
- [x] Compléter [`2026-06-09.md`](./2026-06-09.md)
...
```mermaid
flowchart TD
    done[PartSmart clos] --> A[Commits lundi + journal 09]
    A --> B[credentials include header]
