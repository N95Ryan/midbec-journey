...
- Routine réseau hebdo acceptée avec Patrick (déjeuner vendredi + contact SFE Toronto) — alignée avec l’objectif North Star

### Côté technique

- i18n compatible React 19 sur tout le code applicatif — migration `FormattedMessage` → `intl.formatMessage`, plus d’usage JSX problématique dans `Sources/src`
- Types du menu mobile alignés sur les refs React — `containerRef` corrigé (`RefObject` cohérent avec `useRef<HTMLDivElement>(null)`)
- Build toujours rouge mais causes cartographiées — conflit `ReactPortal` / `ReactNode` et double `@types/react` ; correctifs typage/cast tentés sur `MobileMenuPanelController.tsx`, plan d’action noté pour lundi
