...
- Débloquer le compte de service `scan@midbec.com` (rôle Admin)
...
- Création de `src/app/[locale]/recherche-par-modele/page.tsx`
...
- Branche `feat/partsmart-embed-page` pushée sur `midbec-front`

### Scope 8 — Client Go PartSmart (ajustement)

_Ajout des champs de profil dans PortalUserToken_

- Ajout de `firstName`, `lastName`, `businessName` dans `portalUserTokenRequest`
- Mise à jour de la signature de `PortalUserToken()`
...
- Compte `scan@midbec.com` créé avec rôle User — insuffisant pour l'API
- Mail envoyé à PartSmart pour changer le rôle en Admin
- Temporairement : `rpina@midbec.com` utilisé en dev
...
- Attendre réponse de PartSmart sur le rôle de `scan@midbec.com`
