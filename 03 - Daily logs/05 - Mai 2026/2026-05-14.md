...
## 💡 Découverte du jour
Le site legacy Midbec n'appelle pas l'API PartSmart directement — il appelle son propre backend RPC (`getPlansOfModels`) qui interroge une base de données interne synchronisée avec PartSmart. Il n'y a probablement pas d'API REST publique consommable côté LeadVenture.
...
*Diagnostic du retour vide sur /api/search/en/models*
...
- Le token `/partsmart/token` (admin) ≠ token portal utilisateur — ne pas les confondre dans les tests curl
- `e.stopPropagation()` obligatoire quand un bouton est dans un élément cliquable parent

## ⏭️ Next steps
- [ ] Attendre réponse LeadVenture : API REST PartSmart + rôle Admin scan@midbec.com
